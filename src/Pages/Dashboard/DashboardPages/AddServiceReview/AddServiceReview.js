...
const AddServiceReview = () => {
    ...
    useEffect(() => {
        ...
        axios.get('https://service-assistant.adaptable.app/singleservice').then(res => {
    ...
    const onSubmit = data => {
        ...
        if (!matchReviews) {
            setLoading(false);
            axios.post(`https://service-assistant.adaptable.app/singleservice/addreview/${id}`, { ...data, rating: value, date, id: user.uid, serviceId: id }).then(() => {
                reset();
                setAlert(true);
                setLoading(true);
                const { user, review, rating, date, uid, userPhoto } = data;
                axios.post(`https://service-assistant.adaptable.app/providerdetials/addreview?email=${providerEmail.providerEmail}`, { rating: value, date, uid: uid, serviceId: id, userName: user, userRating: rating, userComment: review, userPhoto, });
            });
        } else {
            UpdateReview(data);
        }
    ...
    const UpdateReview = (data) => {
        ...
        axios.put(`https://service-assistant.adaptable.app/singleservice/updatereview/${user.uid}`, { ...data, rating: value, serviceId: id }).then(() => {
            reset();
            setUpdateing(false);
        });
        // }, [updating])
    ...
    const DeleteReview = () => {
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    axios.delete(`https://service-assistant.adaptable.app/singleservice/deleteReview?parentId=${id}&&uid=${user.uid}`).then(res => {
                        setDeleting(false);
