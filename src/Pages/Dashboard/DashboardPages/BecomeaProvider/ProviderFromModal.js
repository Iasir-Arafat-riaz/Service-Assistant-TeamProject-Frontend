...
const ProviderFromModal = ({ handleOpenModal, open, handleCloseModal, id, category }) => {
    ...
    const onSubmit = data => {
        ...
        if (user.role !== 'provider') {
            axios.post('https://service-assistant.adaptable.app/addprovider', { ...category, data, date: new Date(), rating: 0, reviewUser: 0, backgroundImage: 'https://i.ibb.co/RjGqhfx/photo-1524334228333-0f6db392f8a1-1.webp' }).then(() => {
                reset();
                handleCloseModal();
                // setLoading(true);
            });
    ...
    const addService = () => {
        setLoading(false)
        axios.post(`https://service-assistant.adaptable.app/providerdetials/addservice/${user.email}`, category).then(res => {
            setLoading(true);
            handleCloseModal();
        });
    };
